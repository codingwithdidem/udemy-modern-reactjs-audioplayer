    ...
    <button
      onClick={() => {
        const currentIndex = playbackRates.findIndex(
          (rate) => rate.value === playbackRate.value
        );
        const nextIndex = (currentIndex + 1) % playbackRates.length;
        setPlaybackRate(playbackRates[nextIndex]);
        onPlaybackRateChange(playbackRates[nextIndex].value);
      }}
      className="p-2 rounded-full bg-white/10 backdrop-blur-sm hover:bg-white/20 text-black"
    >
      <playbackRate.icon className="h-5 w-5" />
    </button>
  );
}
